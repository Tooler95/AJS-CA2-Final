...
componentDidMount() {
 axios.get(process.env.REACT_APP_TVGUIDE + '/updates/')
  .then(response => {
...
removeUpdate(id)
{
  ...
  if (r == true){
  axios.delete(process.env.REACT_APP_TVGUIDE + '/update/'+id)
...
deleteUpdate(id)
{
  ...
  if (r == true){
  axios.delete(process.env.REACT_APP_TVGUIDE + '/updates/'+id)
    .then(res => console.log(res.data));
  this.setState({
    updates: this.state.updates.filter(el => el._id !== id)
  })
